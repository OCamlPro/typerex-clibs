...
let grow_buffer s =
  let s' = Bytes.create (2 * Bytes.length s + 1) in
...
let compress_string ?(level = 6) inbuf =
  let inbuf = Bytes.of_string inbuf in
(*  Printf.fprintf stderr "compress_string (%d)...\n%!" (String.length inbuf); *)
  ...
  deflate_end zs;
(*  Printf.fprintf stderr "compress_string done...\n%!"; *)
  res
...
let gzip_string ?(level = 6) inbuf =
  ...
  let write_int32 wbuf n =
    let r = ref n in
    for i = 1 to 4 do
      Buffer.add_char wbuf (char_of_int (Int32.to_int (Int32.logand !r (Int32.of_int 255))));
...
let uncompress_string s =
  (*  Printf.fprintf stderr "uncompress_string...\n%!"; *)
  ...
  uncompress ~header: true (fun b ->
      ...
      Bytes.blit_string s !pos b 0 n;
      pos := !pos + n;
      n end
  ) (fun s len -> Buffer.add_string buf (Bytes.sub_string s 0 len));
(*  Printf.fprintf stderr "uncompress_string done\n%!"; *)
  Buffer.contents buf
