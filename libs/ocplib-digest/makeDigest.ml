...
module Make(S : sig
  ...
  val context_finish : bytes -> context -> unit
  ...
  let context_finish ctx =
    let ctx = Bytes.copy ctx in
    let digest = Bytes.create length in
...
module HMAC ( H : DigestSig) = struct
    ...
    let compute key text =
      let key =
        ...
        if len < 64 then
            let s = Bytes.make 64 '\000' in
            Bytes.blit_string key 0 s 0 len;
            Bytes.to_string s
        else
